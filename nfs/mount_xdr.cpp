...
bool_t xdr_exports(XDR *xdrs, exportnode **objp)
{
   register long *buf=buf;
...
bool_t xdr_ppathcnf(XDR *xdrs, ppathcnf *objp)
{
   ...
   {
      buf = (long int*)XDR_INLINE(xdrs,6 * BYTES_PER_XDR_UNIT);
      if (buf == NULL)
      {
      ...
      {
/*         *__extension__((u_int32_t*)(buf)) = (long)htonl((u_long)(objp->pc_link_max));
         buf++;*/
      ...
      buf = (long int *)XDR_INLINE(xdrs,   2  * BYTES_PER_XDR_UNIT);
      if (buf == NULL)
      {
