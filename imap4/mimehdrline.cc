...
mimeHdrLine::mimeHdrLine(mimeHdrLine *aHdrLine) :
	mimeValue( aHdrLine->mimeValue ),
	mimeLabel( aHdrLine->mimeLabel )
{
}
...
int mimeHdrLine::appendStr(const char *inCStr){
	...
	if(aCStr)
	{
			...
			if(skip && !mimeLabel.isEmpty())
			{
				...
				if(mimeLabel.isEmpty()) return setStr(aCStr);
			}
	}
	return retVal;
}
...
//	char *begin = aCStr;	
	...
	if(aCStr)
	{
		...
		{
  		int label=0,advance;
  		while((advance = parseWord(&aCStr[label]))) {label += advance;}
  		if(label && aCStr[label-1] != ':') retVal = 0;
  		else {
...
int mimeHdrLine::parseWord(const char *inCStr)
{
	...
	if(aCStr && *aCStr) {
		if(*aCStr == '"') return mimeHdrLine::parseQuoted('"','"',aCStr);
		else return mimeHdrLine::parseHalfWord(aCStr);
	} else {
		//debug();
	}
	return retVal;
...
/** slurp one word*/
int mimeHdrLine::parseQuoted(char startQuote,char endQuote,const char *inCStr)
{
	char *aCStr = (char *)inCStr;
	int retVal=0;
		
	if(aCStr && *aCStr) {
		if(*aCStr == startQuote) { aCStr++;retVal++; }
		else return 0;
		...
		if(*aCStr == endQuote) { aCStr++;retVal++; }
	} else {
		//debug();
	}
	return retVal;
...
int mimeHdrLine::parseHalfWord(const char *inCStr)
{
	...
	if(aCStr && *aCStr) {
		if(isalnum(*aCStr)) return mimeHdrLine::parseAlphaNum(aCStr);
			//skip over backticks
			if(*aCStr == '\\') {
  			aCStr++;retVal++;
...
int mimeHdrLine::skipWS(const char *inCStr)
{
	...
	if(aCStr && *aCStr) {
		while(*aCStr == ' ' || *aCStr == '\t') { aCStr++; retVal++; }
		//check out for continuation lines
		if(*aCStr == '\r') {aCStr++;retVal++;}
		if(*aCStr++ == '\n')
		if(*aCStr == '\t' || *aCStr == ' ' )
		{
...
/** parses continuated lines */
int mimeHdrLine::parseSeparator(char separator,const char *inCStr)
{
	char *aCStr = (char *)inCStr;
	int retVal=0;
	int skip;
...
//		qDebug("mimeHdrLine::parseDate - state %d - remaining [%d] '%s'",state,skip,aCStr);
    	...
    	case 1:
    	  ...
    	  break;
    	case 2:
    	  if (!qstrnicmp(aCStr,"Jan",3)) { fillTime->tm_mon = 0; break; }
    	  if (!qstrnicmp(aCStr,"Feb",3)) { fillTime->tm_mon = 1; break; }
    	  if (!qstrnicmp(aCStr,"Mar",3)) { fillTime->tm_mon = 2; break; }
    	  if (!qstrnicmp(aCStr,"Apr",3)) { fillTime->tm_mon = 3; break; }
    	  if (!qstrnicmp(aCStr,"May",3)) { fillTime->tm_mon = 4; break; }
    	  if (!qstrnicmp(aCStr,"Jun",3)) { fillTime->tm_mon = 5; break; }
    	  if (!qstrnicmp(aCStr,"Jul",3)) { fillTime->tm_mon = 6; break; }
    	  if (!qstrnicmp(aCStr,"Aug",3)) { fillTime->tm_mon = 7; break; }
    	  if (!qstrnicmp(aCStr,"Sep",3)) { fillTime->tm_mon = 8; break; }
    	  if (!qstrnicmp(aCStr,"Oct",3)) { fillTime->tm_mon = 9; break; }
    	  if (!qstrnicmp(aCStr,"Nov",3)) { fillTime->tm_mon = 10; break; }
    	  if (!qstrnicmp(aCStr,"Dec",3)) { fillTime->tm_mon = 11; break; }
    	  return 0;
    	case 3:
    	  if (!flagnumeric) return 0;
...
//    	    if (!qstrnicmp(aCStr,"-0000",5)) known = 1;
    	  ...
    	  if (!qstrnicmp(aCStr,"UT",2)) { ct_offset = 0; break; }
    	  if (!qstrnicmp(aCStr,"GMT",3)) { ct_offset = 0; break; }
    	  /* XXX: GMT+nnnn? */

    	  if (!qstrnicmp(aCStr,"BST",3)) { ct_offset = 60; break; }
    	  if (!qstrnicmp(aCStr,"CDT",3)) { ct_offset = -300; break; }
    	  if (!qstrnicmp(aCStr,"CET",3)) { ct_offset = 60; break; }
    	  if (!qstrnicmp(aCStr,"CST",3)) { ct_offset = -360; break; }
    	  if (!qstrnicmp(aCStr,"EDT",3)) { ct_offset = -240; break; }
    	  if (!qstrnicmp(aCStr,"EET",3)) { ct_offset = 120; break; }
    	  if (!qstrnicmp(aCStr,"EST",3)) { ct_offset = -300; break; }
    	  if (!qstrnicmp(aCStr,"HKT",3)) { ct_offset = 480; break; }
    	  if (!qstrnicmp(aCStr,"IST",3)) { ct_offset = 120; break; }
    	  if (!qstrnicmp(aCStr,"JST",3)) { ct_offset = 540; break; }
    	  if (!qstrnicmp(aCStr,"MDT",3)) { ct_offset = -360; break; }
    	  if (!qstrnicmp(aCStr,"MET",3)) { ct_offset = 60; break; }
    	  if (!qstrnicmp(aCStr,"METDST",6)) { ct_offset = 120; break; }
    	  if (!qstrnicmp(aCStr,"MST",3)) { ct_offset = -420; break; }
    	  if (!qstrnicmp(aCStr,"PDT",3)) { ct_offset = -420; break; }
    	  if (!qstrnicmp(aCStr,"PST",3)) { ct_offset = -480; break; }
    	  return 0;
    	case 10:
    	  if (!qstrnicmp(aCStr,"DST",3)) ct_offset += 60;
    	  break;
    }
    if (state < 10) ++state;
...
QCString mimeHdrLine::truncateLine(QCString aLine,unsigned int truncate)
{
	int cutHere;
	QCString retVal;
