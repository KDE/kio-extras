...
QString KSSHProcess::version() {
    ...
    if( (p = popen(cmd.latin1(), "r")) == NULL ) {
        kdDebug(KSSHPROC) << "KSSHProcess::version(): failed to start ssh: " << strerror(errno);
        return QString::null;
    }
    ...
    if( (len = fread(buf, sizeof(char), sizeof(buf)-1, p)) == 0 ) {
        kdDebug(KSSHPROC) << "KSSHProcess::version(): Read of ssh version string failed " << strerror(ferror(p));
        return QString::null;
    }
    pclose(p);
    buf[len] = '\0';
    QString ver;
    ver = buf;
    kdDebug(KSSHPROC) << "libssh::version(): got version string [" << ver << "]";
    ...
    if( mVersion == -1 ) {
        kdDebug(KSSHPROC) << "KSSHProcess::version(): Sorry, I don't know about this version of ssh";
        return QString::null;
    }
...
bool KSSHProcess::setArgs(const SshOptList& opts) {
    ...
    // These options govern the behavior of ssh and are not user defined
    mArgs.append("-o");
...
int KSSHProcess::error(QString& msg) {
    if( !msg.isNull() ) {
        // assign message describing error based on the err number
    }
    return mError;
}
...
bool KSSHProcess::connect() {
    ...
    int maxfd = ptyfd > iofd ? ptyfd : iofd;  // find max file descriptor
    maxfd = maxfd > errfd ? maxfd : errfd;
    ...
    do {
        ...
        if( ret > 0 ) {
            ...
            if( FD_ISSET(errfd, &rfds) ) {
                QCString line; line.resize(1024);
                int len;
                len = ::read(errfd, line.data(), line.size() - 1);
                switch( len ) {
                case -1:
                    if( errno != EINTR || errno != EAGAIN ) {
                        kdDebug(KSSHPROC) << "KSSHProcess::connect(): stderr read error " <<
                                             strerror(errno) << endl;
                        mError = ERR_INTERNAL;
                        return false;
                    }
                case 0:
                    kdDebug(KSSHPROC) << "KSSHProcess::connect(): read eof on stderr" << endl;
                    mError = ERR_DISCONNECTED;
                    return false;
                default:
                    line[len] = '\0';
                }
                kdDebug(KSSHPROC) << "KSSHProcess::connect(): got line from stderr [" << line << "]" << endl;
