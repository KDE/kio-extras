...
char *SMBIO::getString(const char* message, bool echo)
{
	if (getObjectCallback)
		return getObjectCallback->getAnswer(message, echo);
	else if (getFuncCallback)
		return getFuncCallback(message, echo);
	return 0;
}
...
//	if (errno!=ERRbadpw) {delete user; delete password; return -1;}
	...
	if ((!user) || (!password))
	{
		if (!user) user=getString("User",true);
		if (!password) password=getString("Password",false);
		if (doLogin(user, password, UID)!=-1) {delete user; delete password; return 1;}
		if (errno!=ERRbadpw) {delete user; delete password; return -1;}
	}
	int l1=strlen(user);
...
*/
			...
			strcpy(msg,"Password for service ");
			strcpy(msg+21,service);
			char *password2=getString(msg, false);
			delete msg;
...
// I will do the parsing here, not in a lex file.
// This way, there is no external library dependency, and I can introduce
// a limited AI in this parser using SMBIO variables
	...
	char* &dir, char* &user)
{
	...
	len=strlen(name);
	str=new char[len+1];	// string to parse
	strsav=str;	// save pointer for delete;
	strcpy(str,name);
	if(char_cnv) // Do some conversion.
	  char_cnv->unix2win(str);
	// first convert all the infamous \ in unix-style /
	for (i=0; i<len; i++) if (*(str+i)=='\\') *(str+i)='/';
	if ((p=strchr(str,':'))) {	// URL-like protocol specified
		if ((p-str!=3)			// must be smb (case insensitive)
		|| (toupper(str[0])!='S')
		|| (toupper(str[1])!='M')
		|| (toupper(str[2])!='B')) {
			errno=ENOTDIR;
		...
		strcpy(str,p+1);	// remove protocol field
		len-=4;
		// now check wether there is a user/workgroup specified
		if (!len) goto PARSEDONE; // anonymous browsing
		if ((p=strchr(str,'@'))) { // look for an @ => user specified
			user=new char[p-str+1];
			memcpy(user,str,p-str+1);
			user[p-str]=0; // remove the '@' and put 0 at the end
			strcpy(str,p+1); // remove user and @
			len=strlen(str);
			if (!len) goto PARSEDONE; // user but nothing else => browsing
		}
		
		if ((len>0) && (str[0]!='/')) { // yes, this should be a workgroup
			p=strchr(str,'/');	// look for the next /
			if ((p) && (p[1]==0)) { // yes, and it was a trailing /
				p[0]=0; // for strcpy
				p=0;	// ignore it
			}
			if (!p) { // only a workgroup : we'll browse it then
				len=strlen(str);
				if (!len) goto PARSEDONE; // well, no workgroup finally :-)
				workgroup=new char[len+1];
				strcpy(workgroup,str);
				goto PARSEDONE;	// He/she who wants endless 'if's can code it !
			}
			workgroup=new char[p-str+1];
			memcpy(workgroup,str,p-str+1);
			workgroup[p-str]=0; // remove the '/' in workgroup name
			strcpy(str,p);	// remove workgroup field, keep the /
			len=strlen(str);
		}
	}
	// now check //[user@][workgroup/]host
	if ((len>=2) && (str[0]=='/')) {
		strcpy(str,str+1);	// remove the slash, handle smb:/ and smb://
		if (str[0]=='/') strcpy(str,str+1); // remove the slash
		if ((p=strchr(str,'@'))) { // look for an @ => user specified
			if (user) delete user; // was already specified, take the second
			user=new char[p-str+1];
			memcpy(user,str,p-str+1);
			user[p-str]=0; // remove the '@' and put 0 at the end
			strcpy(str,p+1); // remove user and @
		...
		p=strchr(str,'/');	// look for the next one
		if ((p) && (p[1]==0)) { // check if it was a trailing /
			p[0]=0; // for strcpy
			p=0;	// ignore it
		}
		if (!p) {	// just an host, directory content will be the share list
			len=strlen(str);
			if (!len) goto PARSEDONE; // browsing required, host already 0
			host=new char[len+1];
			strcpy(host,str);
		} else {
			host=new char[p-str+1];
			memcpy(host,str,p-str+1);
			host[p-str]=0;
			strcpy(str,p);	// remove host field, keep the / to indicate share
			len=strlen(str);
		}
		// suppose it's an host if workgroup already specified
		if (!workgroup) {
			// now we must check if it's an host or a workgroup
			getWorkgroupList();
			int flag=0;
			for (SMBWorkgroupList *wk=workgroupList; (wk); wk=wk->next)
				if (!strcasecmp(wk->name,host)) {flag=1; break;}
			if (flag) { // a workgroup has this name
				workgroup=host; // workgroup was null
				host=0;
				if (!p) goto PARSEDONE; // no more fields
				if (str[0]=='/') strcpy(str,str+1); // remove the slash
				p=strchr(str,'/');	// look for the next one
				if ((p) && (p[1]==0)) { // check if it was a trailing /
					p[0]=0; // for strcpy
	...
	if ((len>=1) && (str[0]=='/')) { // can be either a share or a directory
		if ((!host) && (currentShare)) { // previous field not present, share open...
			dir=new char[len+1];         // it's probably a directory
			strcpy(dir,str); // and the / make it from share root : ok
			goto PARSEDONE;
		} else {	// consider it was a share
			strcpy(str,str+1);	// remove '/'
			p=strchr(str,'/');	// look for the next one
			if ((p) && (p[1]==0)) { // check if it was a trailing /
				p[0]=0; // for strcpy
				p=0;	// ignore it
			...
			p[0]=0;	// stop the copy here
			share=new char[p-str+1];
			strcpy(share,str);
			strcpy(str,p+1);	// remove share field
			len=strlen(str);
		}
	}
	if (str) { // only directory should remain here
		dir=new char[len+1];
		strcpy(dir,str);
		goto PARSEDONE;
	}
...
#if DEBUG >= 4
	cout<<"parse : \n";
	if (workgroup) cout<<"    workgroup : "<<workgroup<<"\n";
	if (host) cout<<"    host : "<<host<<"\n";
	if (share) cout<<"    share : "<<share<<"\n";
	if (dir) cout<<"    dir : "<<dir<<"\n";
...
{
	if ((!share) && (file)) {
		if (!currentShare) return 0; // file without localisation : wrong URL
		share=currentShare; // dir/file relative to opened share
	}
	if ((!host) && (share)) {
		if (!hostName) return 0; // share without localisation : wrong URL
		host=hostName; // share supposed from opened host
	}
	int len=6; // smb://
	if (user) len+=strlen(user)+1;         // smb://user@
	if (workgroup) len+=strlen(workgroup); // smb://user@workgroup
	if (host) len+=strlen(host)+1;         // smb://user@workgroup/host
	if (share) len+=strlen(share)+1; // smb://user@workgroup/host/share
	if (file) len+=strlen(file)+1;   // smb://user@workgroup/host/share/file
	char *ret=new char[len+1]; // 0 at the end
	char *str=ret;  // string pointer we can work on and modify
	strcpy(str,"smb:/"); str+=5; // smb:/
	if (user) {
		strcpy(str,"/"); str++;            // smb://
		strcpy(str,user); str+=strlen(user); // smb://user
		strcpy(str,"@"); str++;              // smb://user@
	}
	if (workgroup) {
		strcpy(str,"/"); str++;            // smb://
		strcpy(str,workgroup);
		str+=strlen(workgroup); // smb://user@workgroup
	}
	if (host) {
		strcpy(str,"/"); str++;            // smb://user@workgroup
		strcpy(str,host); str+=strlen(host); // smb://user@workgroup/host
	}
	if (share) {
		strcpy(str,"/"); str++;            // smb://user@workgroup/host/
		strcpy(str,share); str+=strlen(share); // smb://user@workgroup/host/share
	}
	if (file) {
		strcpy(str,"/"); str++;   // smb://user@workgroup/host/share/
		strcpy(str,file); str+=strlen(file); // smb://user@workgroup/host/share/file
	}
	if (!strcmp(ret,"smb:/")) strcpy(str,"/");
#if DEBUG >= 4
	cout<<"buildURL : "<<ret<<"\n";
#endif
	return ret;	
...
{
	if (!string) {
		char *ret=new char[strlen(URL)+1];
		strcpy(ret,URL);
		return ret;
	}
	if (!strcmp(string,".")) { // no need to go further on
		if (!URL) return 0;
		char *ret=new char[strlen(URL)+1];
		strcpy(ret,URL);
		return ret;
	...
	char *dir=0;
	char *user=0;
	if (parse(URL,workgroup,host,share,dir,user)==-1) {
		if (workgroup) delete workgroup;
		if (host) delete host;
		if (share) delete share;
		if (dir) delete dir;
	...
	if (!strcmp(string,"..")) {
		if (!workgroup) {
			if (!host) {
				if ((share) || (dir)) { // must have host for a share...
					if (share) delete share;
					if (dir) delete dir;
					if (user) delete user;
					errno=ENOENT; return 0;
				}
				goto APPENDDONE; // can't go upper than 'smb:' !
			}
		}
		if (!host) {
			if ((share) || (dir)) { // must have host for a share...
			...
			if (workgroup) delete workgroup;
			workgroup=0;  // go one level up if possible
			goto APPENDDONE; // can't go upper than 'smb:' !
		}
		if (!share) {
			if (dir) { // must have share for a dir !
				if (workgroup) delete workgroup;
				if (host) delete host;
				if (dir) delete dir;
				if (user) delete user;
			...
			if (host) delete host;
			host=0;   // ok, host wasn't 0 before
			goto APPENDDONE; // can't go upper than 'smb:' !
		}
		if ((!dir) || (!strcmp(dir,"")) || (!strcmp(dir,"/"))) {
			if (share) delete share;
			share=0; goto APPENDDONE; // ok, share wasn't 0 before
		}
		// so now it's a _true_ drectory on a server, and not the root one
		// check if it has subdirs, and remove the last one if any
		p=strrchr(dir,'/');
		if ((!p) || (dir[0]=='/')) { // no subdirs
			if (dir) delete dir;
			dir=0; goto APPENDDONE;   // go to root dir
		}
		len=p-dir;
		p=new char[len+1]; // make a copy we can work on
		memcpy(p,dir,len);
		p[len]=0;          // remove '/'
		delete dir;
		dir=p;             // new value
	...
	if (!workgroup) {
		if (!host) {
			...
			workgroup=new char[strlen(string)+1];
			strcpy(workgroup,string);     // or whatever it is
			goto APPENDDONE;
		}
	}
	if (!host) {
		...
		strcpy(host,string);     // or whatever it is
		goto APPENDDONE;
	}
	if (!share) {
		if (dir) { // must have share for a dir !
		...
		share=new char[strlen(string)+1];
		strcpy(share,string);     // or whatever it is
		goto APPENDDONE;
	}
	if (!dir) {
		dir=new char[strlen(string)+1];
		strcpy(dir,string);     // or whatever it is
		goto APPENDDONE;
	}
	// so now it's a _true_ drectory on a server
	// append subdir, with / before
	p=new char[strlen(dir)+strlen(string)+2];
	strcpy(p,dir);
	strcpy(p+strlen(dir),"/");
	strcpy(p+strlen(dir)+1,string);
	delete dir;
	dir=p;
APPENDDONE:
	char *url=buildURL(workgroup,host,share,dir,user);
	if (!url) {
		if (workgroup) delete workgroup;
		if (host) delete host;
		if (share) delete share;
	...
	// now parse it and build it again so as to be sure it is correct
	if (workgroup) delete workgroup;
	workgroup=0;
	if (host) delete host;
	host=0;
	...
	delete url;
	url=buildURL(workgroup,host,share,dir,user);
	if (workgroup) delete workgroup;
	if (host) delete host;
	if (share) delete share;
	if (dir) delete dir;
