...
class ServiceTypeRepository : virtual public CORBA::Object
{
  public:
    ...
    virtual IncarnationNumber add_type( const char* name, const char* if_name, const PropStructSeq& props, const ServiceTypeNameSeq& super_types, const PropertySeq& values ) = 0;
    virtual void remove_type( const char* name ) = 0;
    virtual ServiceTypeNameSeq* list_types( const SpecifiedServiceTypes& which_types ) = 0;
    virtual TypeStruct* describe_type( const char* name ) = 0;
    virtual TypeStruct* fully_describe_type( const char* name ) = 0;
...
// Stub for interface ServiceTypeRepository
class ServiceTypeRepository_stub : virtual public ServiceTypeRepository
{
  public:
    virtual ~ServiceTypeRepository_stub();
    IncarnationNumber incarnation();
    IncarnationNumber add_type( const char* name, const char* if_name, const PropStructSeq& props, const ServiceTypeNameSeq& super_types, const PropertySeq& values );
    void remove_type( const char* name );
    ServiceTypeNameSeq* list_types( const SpecifiedServiceTypes& which_types );
    TypeStruct* describe_type( const char* name );
    TypeStruct* fully_describe_type( const char* name );
    void mask_type( const char* name );
    void unmask_type( const char* name );
  private:
    void operator=( const ServiceTypeRepository_stub& );
};
