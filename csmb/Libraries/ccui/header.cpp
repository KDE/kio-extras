...
struct CHeaderData
{
	...
	for ( int i = 0; i < n ; i ++ ) {
	    sizes[i] = 88;
	    // heights[i] = 10; set properly in CHeader::init()
	    i2s[i] = i;
	    s2i[i] = i;
	    positions[i] = p;
	    p += sizes[i];
...
/*!
  \class CHeader header.h
  \brief The CHeader class provides a table header.
  \ingroup advanced

  This class provides a table header as known from Spreadsheet-like
  widgets. CHeader can be used vertically or horizontally (see
setOrientation()).
  ...
  the API of CHeader works with the section numbers.
...
CHeader::CHeader( QWidget *parent, const char *name,
        bool bUseHeaderExtender ) :
    QWidget( parent, name, WNorthWestGravity )
{
    orient = Horizontal;
    init( 0, bUseHeaderExtender );
}
...
CHeader::CHeader( int n,  QWidget *parent, const char *name,
        bool bUseHeaderExtender ) :
    QWidget( parent, name, WNorthWestGravity )
{
    orient = Horizontal;
    init( n, bUseHeaderExtender );
}
...
/*!
  \fn void CHeader::sizeChange( int section, int oldSize, int newSize )
...
/*!
  \fn void CHeader::clicked( int section )
...
/*!
  \fn void CHeader::pressed( int section )
...
/*!
  \fn void CHeader::released( int section )
...
/*!
  \fn void CHeader::indexChange( int section, int fromIndex, int toIndex )
...
/*!
  \fn void CHeader::moved( int fromIndex, int toIndex )
  \obsolete
...
/*!
  \fn void CHeader::sectionClicked( int index )
  \obsolete
...
int CHeader::cellSize( int i ) const
{
...
int CHeader::cellPos( int i ) const
{
    if ( i == count() && i > 0 )
...
/*!
  \fn Orientation CHeader::orientation() const
...
/*!
  \fn void CHeader::setTracking( bool enable )
...
/*!
  \fn bool CHeader::tracking() const
 ...
 */
void CHeader::init( int n, bool bUseHeaderExtender )
{
    state = Idle;
    offs = 0;
    cachedIdx = 0; // unused
    ...
    // new for Corel
    d->bUseHeaderExtender = bUseHeaderExtender;

}

/*!
  Sets the header orientation.  The \e orientation must be
  CHeader::Vertical or CHeader::Horizontal.
...
void CHeader::setOrientation( Orientation orientation )
{
...
/*!
  ...
  */
void CHeader::paintRect( int p, int s )
{
    QPainter paint( this );
...
/*!
  ...
  */
void CHeader::unMarkLine( int idx )
{
    if ( idx < 0 )
	return;
    int p = pPos( idx );
...
int CHeader::cellAt( int pos ) const
{
...
void CHeader::moveCell( int fromIdx, int toIdx )
{
...
/*!
  \reimp
*/
void CHeader::mousePressEvent( QMouseEvent *e )
{
...
/*!
  \reimp
*/
void CHeader::mouseReleaseEvent( QMouseEvent *e )
{
    if ( e->button() != LeftButton )
	return;
...
/*!
  \reimp
*/
void CHeader::mouseMoveEvent( QMouseEvent *e )
{
    int section;
    bool hit;
    ...
    default:
	qWarning( "CHeader::mouseMoveEvent: (%s) unknown state", name() );
	break;
    }
}
...
void CHeader::handleColumnResize( int index, int c, bool final )
{
...
QRect CHeader::sRect( int index )
{
    int section = mapToSection( index );

    // check if this index is Corel's header extender
    if ( section < 0 )
    {
        if ( count() == 0 )
        {
            // header extender takes up entire listview
            return QRect( 0, 0, width(), height() );
        }
        else if ( index == count() )
        {
            // header extender reaches from last column to end of listview
            int lastIndex = count() - 1;
            int lastSection = d->i2s[lastIndex];
            if ( orient == Horizontal )
                return QRect( d->positions[lastIndex] +
                    d->sizes[lastSection] - offset(), 0, width(), height() );
            else
                return QRect( 0, d->positions[lastIndex] +
                    d->sizes[lastSection] - offset(), width(), height() );
        }
        else
...
void CHeader::setLabel( int section, const QIconSet& iconset, const QString
&s, int size ) {
...
void CHeader::setLabel( int section, const QString &s, int size )
{
    if ( section < 0 || section >= count() )
	return;
...
QString CHeader::label( int section ) const
{
    if ( section < 0 || section >= count() )
	return QString::null;
    if ( d->labels[ section ] )
...
QIconSet *CHeader::iconSet( int section ) const
{
    if ( section < 0 || section >= count() )
	return 0;
...
/*!
  ...
  label is used.
*/
int CHeader::addLabel( const QIconSet& iconset, const QString &s, int size )
{
    int n = count() + 1;
...
*/
void CHeader::removeLabel( int section )
{
...
int CHeader::addLabel( const QString &s, int size )
{
    int lastpos = 0;
    if ( d->count )
...
/*!\reimp
*/
QSizePolicy CHeader::sizePolicy() const
{
    if ( orient == Horizontal )
...
*/
int CHeader::pHeight( int i ) const
{
...
*/
void CHeader::setPHeight( int i, int h )
{
    int section = mapToSection(i);
    if ( section < 0 )
	return;
...
int CHeader::mapToLogical( int a ) const
{
...
int CHeader::mapToActual( int l ) const
{
    return mapToIndex( l );
}
...
void CHeader::setCellSize( int section, int s )
{
    if ( section < 0 || section >= count() )
	return;
    d->sizes[ section ] = s;
...
void CHeader::setResizeEnabled( bool enable, int section )
{
    if ( section < 0 ) {
...
void CHeader::setMovingEnabled( bool enable )
{
    d->move = enable;
}
...
void CHeader::setClickEnabled( bool enable, int section )
{
    if ( section < 0 ) {
...
// Modified to handle Corel's header extender
void CHeader::paintSection( QPainter *p, int index, QRect fr )
{
    int section = mapToSection( index );

    // if this index is not a valid section, then it is the header extender
    // if (a) we are actually using a header extender and (b) it is at index 
    // count() (since the last column is index count()-1)

    // if it is an invalid section and not the header extender, return
    if ( section < 0 && (!d->bUseHeaderExtender || index != count()) )
	  return;

    bool down;
    if ( section < 0)
      down = false;    // header extender is never pressed
    else
      down = (index==handleIdx) && ( state == Pressed || state == Moving );
    p->setBrushOrigin( fr.topLeft() );

    // simply draw a bevelled button if either:
    //   - this is the header extender and
    //      - there are no columns or
    //      - the last column is clickable
    //   - this is a regular column and
    //      - it is clickable
    // otherwise, draw a flat panel
    
    if ( (section < 0 && ( count() == 0 || d->clicks[ count()-1 ] )) ||
         (section >= 0 && d->clicks[section]) ) {
    ...
    // if this is not the header extender, draw the label
    if ( section >= 0 )
...
*/
void CHeader::paintSectionLabel( QPainter *p, int index, const QRect& fr )
{
    int section = mapToSection( index );
    if ( section < 0 )
	return;
    ...
    // Draw arrows in Corel style
    int tw = p->fontMetrics().width( s ) + 16;
    if ( d->sortColumn == section && pw + tw + 8 < fr.width() )
    {
    	//int nSize = height() / 3;
    	int nY = (height() - 7) / 2;
    	int nX = fr.x() + pSize( index ) - 7 - nY;
    	if ( (index==handleIdx) && ( state == Pressed || state == Moving ) )
    	{
    		// position them properly when pressed button pressed down
    		nX++;
    		nY++;
    	}
	p->save();
	if ( d->sortDirection ) {
    		// point
    		p->drawPoint(nX + 3, nY + 6);
    		
    		p->drawPoint(nX + 2, nY + 5);
    		p->drawPoint(nX + 4, nY + 5);
    		p->drawPoint(nX + 2, nY + 4);
    		p->drawPoint(nX + 4, nY + 4);
    		
    		p->drawPoint(nX + 1, nY + 3);
    		p->drawPoint(nX + 5, nY + 3);
    		p->drawPoint(nX + 1, nY + 2);
    		p->drawPoint(nX + 5, nY + 2);
    		
    		p->drawPoint(nX + 0, nY + 1);
    		p->drawPoint(nX + 6, nY + 1);
    	...
    	else
    	{
    		...
    		p->drawPoint(nX + 2, nY + 1);
    		p->drawPoint(nX + 4, nY + 1);
    		p->drawPoint(nX + 2, nY + 2);
    		p->drawPoint(nX + 4, nY + 2);
    		
    		p->drawPoint(nX + 1, nY + 3);
    		p->drawPoint(nX + 5, nY + 3);
    		p->drawPoint(nX + 1, nY + 4);
    		p->drawPoint(nX + 5, nY + 4);
    		
    		p->drawPoint(nX + 0, nY + 5);
    		p->drawPoint(nX + 6, nY + 5);

    		// bottom    		
    		p->drawPoint(nX + 0, nY + 6);
    		p->drawPoint(nX + 1, nY + 6);
    		p->drawPoint(nX + 2, nY + 6);
    		p->drawPoint(nX + 3, nY + 6);
    		p->drawPoint(nX + 4, nY + 6);
    		p->drawPoint(nX + 5, nY + 6);
    		p->drawPoint(nX + 6, nY + 6);
    	}
	p->restore();
    } // if


// original Qt style arrows
...
/*!\reimp
*/
void CHeader::paintEvent( QPaintEvent *e )
{
    QPainter p( this );
    ...
    // Modified by Corel: section count() is the header extender
...
/*!
  As often the CHeader is used together with a list widget,
  CHeader can indicate a sort order. This is done using an arrow at
...
void CHeader::setSortIndicator( int section, bool increasing )
{
...
void CHeader::resizeSection( int section, int s )
{
    if ( section < 0 || section >= count() )
	return;
    d->sizes[ section ] = s;
...
int CHeader::sectionSize( int section ) const
{
    if ( section < 0 || section >= count() )
	return 0;
...
int CHeader::sectionPos( int section ) const
{
    if ( section < 0 || section >= count()  )
	return 0;
...
int CHeader::sectionAt( int pos ) const
{
...
int CHeader::mapToSection( int index ) const
{
...
int CHeader::mapToIndex( int section ) const
{
...
void CHeader::moveSection( int section, int toIndex )
{
...
bool CHeader::isClickEnabled( int section ) const
{
    if ( section >= 0 && section < count() ) {
...
bool CHeader::isResizeEnabled( int section ) const
{
    if ( section >= 0 && section < count() ) {
...
bool CHeader::isMovingEnabled() const
{
    return d->move;
}
...
bool CHeader::moveHeader( int nFromIndex, int nToIndex )
{
    bool bResult;
    ...
    else if (nFromIndex == nToIndex || nFromIndex < 0 || nToIndex < 0 ||
            nFromIndex >= count() || nToIndex >= count())
    {
        bResult = false;
    }
    else
    {
        int nSection = mapToSection(nFromIndex);
        moveSection( nSection, nToIndex+1 );
	    // moveSection index numbers are odd
        emit moved( nFromIndex, nToIndex );	// deprecated
        emit indexChange( nSection, nFromIndex, nToIndex );
        repaint();
        bResult = true;
    }
...
void CHeader::resetHeaderPositions()
{
    for (int nLoop=0; nLoop < count(); nLoop++)
    {
        moveHeader( mapToIndex(nLoop), nLoop);
    }
}
