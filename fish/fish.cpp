...
void fishProtocol::manageConnection(const QString &l) {
	...
	if (!rc) {
		...
		case FISH_RETR:
			...
			break;
		case FISH_STOR:
			if (!checkExist && putPerm > -1) sendCommand(FISH_CHMOD,QString::number(putPerm,8).latin1(),url.path().latin1());
		case FISH_WRITE:
		case FISH_APPEND:
			rawWrite = sendLen;
		...
		if (fishCommand == FISH_FISH) {
			...
			if (listReason == LIST) {
				...
				if (udsStatEntry.size() == 0) error(ERR_DOES_NOT_EXIST,url.path());
...
void fishProtocol::stat(const KURL& u){
	...
	if (!url.hasPath()) {
        ...
        while (path.endsWith("/") && path.length() > 1) path.truncate(path.length()-1);
        wantedFn = path.mid(path.findRev('/')+1);
        if (wantedFn.length() == 0) wantedFn = ".";
        path.truncate(path.findRev('/'));
        if (path.length() == 0) path = "/";
		sendCommand(FISH_LIST,path.latin1());
	}
	run();
}
