...
public:
    ...
    StringDefinition(int len, const char *cstr)
        : m_length(len)
        , m_output(cstr)
    {
    }
...
{
public:
    NumberDefinition(void)
        : m_value(0)
        , m_increment(0)
    {
    }
    NumberDefinition(int value)
        : m_value(value)
        , m_increment(0)
    {
    }
    NumberDefinition(int value, int incr)
        : m_value(value)
        , m_increment(incr)
    {
    }
...
static const StandardName STANDARD_NAMES[] = {{"-ansiC", "ANSI X3.159-1989 ('ANSI C89')"},
...
static const char *const includedirs[] = {"/usr/include",
                                          "/usr/include/sys",
                                          "/usr/local/include",
                                          "/opt/local/include",
                                          "/usr/ccs",
    ...
    */
    ...
    while (nr) {
        ...
        case 4: /* manpage */
            ...
            if (g != nullptr && f > c && (g - f) < 12 && (isalnum(f[-1]) || f[-1] == '>' || (f[-1] == ';')) && (isdigit(f[1]) || (f[1] == 'n')) && f[1] != '0'
                && ((g - f) <= 2 || isalpha(f[2]))) {
                ok = true;
                h = f + 2;
                while (h < g) {
...
static QByteArray scan_named_character(char *&c)
{
    ...
    if (*c == '(') {
        ...
        if (c[1] == escapesym) {
            ...
            // ### HACK: as we convert characters too early to HTML, we need to support more than 2 characters here and assume that all characters passed by the
...
TABLEITEM::TABLEITEM(TABLEROW *row)
    : contents(nullptr)
    , _parent(row)
{
    init();
...
static const char *const tableopt[] = {"center", "expand", "box", "allbox", "doublebox", "tab", "linesize", "delim", nullptr};
...
static const char *const abbrev_list[] = {"GSBG",     "Getting Started ",
...
static const char *const section_list[] = {
    ...
    "DDI and DKI",
    "9E",
    "DDI and DKI Driver Entry Points",
    "9e",
    "DDI and DKI Driver Entry Points",
    "9F",
...
static char *scan_request(char *c)
{
    ...
    if (c[0] == escapesym) {
        ...
        if ((macroName != "SH") && (macroName != "SS") && it != s_stringDefinitionMap.constEnd()) {
            ...
            case REQ_as: // groff (7) "Append String variable"
            {
                ...
                if (it == s_stringDefinitionMap.end()) {
                    ...
                    if (mode) { // .ds Defining String
                        (*it).m_length = curpos;
                        (*it).m_output = result;
                    } else { // .as Appending String
                        (*it).m_length += curpos;
                        (*it).m_output += result;
                    }
                }
            ...
            case REQ_Fo: // mdoc(7) "Function definition Opening"
            {
                ...
                if ((semicolon != nullptr) && (semicolon < eol))
                    *semicolon = ' ';
            ...
            case REQ_TH: // man(7) "Title Header"
            {
                if (!output_possible) {
                    ...
                    out_html("<link rel=\"stylesheet\" href=\"help:/kdoctools6-common/kde-docs.css\"");
                    out_html(" type=\"text/css\">\n");
                    out_html("<link rel=\"stylesheet\" href=\"help:/kdoctools6-common/kde-localised.css\"");
                    out_html(" type=\"text/css\">\n");
                    ...
                    out_html("<div id=\"content\">\n");
                    out_html("<div id=\"header\"><div id=\"header_content\">\n");
                    out_html("<div id=\"header_left\"><div id=\"header_right\">\n");
                    ...
                    out_html("<div id=\"contentBody\">\n");
                    out_html("<div class=\"book\">\n");
                    out_html("<h1 class=\"title\">");
                    if (args.count())
                        scan_troff(args[0].data(), 0, nullptr);
                    out_html("</h1>\n");
                    if (args.count() > 1) {
...
void scan_man_page(const char *man_page)
{
    ...
    if (output_possible) {
        ...
        output_real("</div></div></div></div></div>\n");
