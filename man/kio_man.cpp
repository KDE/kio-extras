...
static QString stripCompression(const QString &name)
{
    ...
    if (name.endsWith(".gz"))
        pos -= 3;
    else if (name.endsWith(".z", Qt::CaseInsensitive))
        pos -= 2;
...
MANProtocol::MANProtocol(const QByteArray &pool_socket, const QByteArray &app_socket)
    : QObject()
    , WorkerBase("man", pool_socket, app_socket)
{
...
char *MANProtocol::readManPage(const char *_filename)
{
    ...
    if (QString(filename).contains("/sman/", Qt::CaseInsensitive)) {
        ...
        if (!QFile::exists(QFile::decodeName(filename))) // if given file does not exist, find with suffix
        {
            ...
            if (entries.isEmpty()) {
                outputError(xi18nc("@info",
                                   "The specified man page references "
...
void MANProtocol::outputHeader(QTextStream &os, const QString &header, const QString &title)
{
    ...
    os << "<link rel=\"stylesheet\" href=\"help:/kdoctools5-common/kde-default.css\" type=\"text/css\">\n";
    if (!m_manCSSFile.isEmpty()) {
        os << "<link rel=\"stylesheet\" href=\"" << m_manCSSFile << "\" type=\"text/css\">\n";
    }
    os << "<style type=\"text/css\">\n";
    os << "#header_top { background-image: url(\"help:/kdoctools5-common/top.jpg\"); }\n";
    os << "#header_top div { background-image: url(\"help:/kdoctools5-common/top-left.jpg\"); }\n";
    os << "#header_top div div { background-image: url(\"help:/kdoctools5-common/top-right.jpg\"); }\n";
    os << "</style>\n";
    os << "</head>\n";

    os << "<body>\n";
    os << "<div id=\"header\"><div id=\"header_top\"><div><div>\n";
    os << "<img src=\"help:/kdoctools5-common/top-kde.jpg\" alt=\"top-kde\">\n";
    os << pageTitle << "\n";
    os << "</div></div></div></div>\n";
...
{
    if (section == "0")
        return i18n("Header Files");
    else if (section == "0p")
        return i18n("Header Files (POSIX)");
    else if (section == "1")
        return i18n("User Commands");
    else if (section == "1p")
        return i18n("User Commands (POSIX)");
    else if (section == "2")
...
void MANProtocol::showMainIndex()
{
    ...
    for (const QString &it_sect : qAsConst(sections)) {
        ...
        os << "<tr><td><a href=\"man:(" << it_sect << ")\" accesskey=\"" << accessKey << "\">" << i18n("Section %1", it_sect) << "</a></td><td>&nbsp;</td><td> "
           << sectionName(it_sect) << "</td></tr>\n";
    }
...
void MANProtocol::constructPath(QStringList &constr_path, QStringList constr_catmanpath)
{
    ...
    // Default paths
    static const char *const manpaths[] = {"/usr/X11/man",     "/usr/X11R6/man", "/usr/man",          "/usr/local/man",     "/usr/exp/man",
                                           ...
                                           "/usr/titools/man", "/usr/sunpc/man", "/usr/ncd/man",      "/usr/newsprint/man", nullptr};
    ...
    if (::getenv("PATH")) {
        const QStringList path = QString::fromLocal8Bit(::getenv("PATH")).split(':', Qt::SkipEmptyParts);
...
void MANProtocol::checkManPaths()
{
    ...
    const bool construct_path = (manpath_env.isEmpty() || manpath_env.startsWith(':') || manpath_env.endsWith(':') || manpath_env.contains("::"));
    ...
    for (const QString &dir : path_list_env) {
        ...
        {
            if (m_manpath.contains(dir))
                continue; // ignore if already present
            if (QDir(dir).exists())
                m_manpath += dir; // add to list if exists
        } else // empty part - merge constructed path
        {
            ...
            for (const QString &dir2 : qAsConst(constr_path)) {
                if (dir2.isEmpty())
                    continue; // don't add a null entry
...
void MANProtocol::showIndex(const QString &section)
{
    ...
    for (int i = 0; i < listlen; i++) {
        ...
        os << manindex->manpage_begin << "</a></td><td>&nbsp;</td><td> "
           << (indexmap.contains(manindex->manpage_begin) ? indexmap[manindex->manpage_begin] : "") << "</td></tr>\n";
        last_index = manindex;
    }
