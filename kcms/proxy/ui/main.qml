...
KCMUtils.SimpleKCM {
    ...
    header: Kirigami.InlineMessage {
        ...
        text: i18nc("@info", "Restart running applications to make these changes take effect for them.")
        visible: false

        Connections {
            target: kcm
            function onShowRestartWarning(): void {
    ...
    Kirigami.Form {
        Kirigami.FormGroup {
            ...
            SystemProxyConfiguration {
                ...
                subtitle: root.i18nContext.i18nc("@placeholder", "Comma-separated list of IP addresses")
        ...
        function updateValue(): void  {
            if (manualRadio.checked) {
                if (urlField.text.length > 0) {
                    configuration.value = urlField.text + (portField.value !== 0 ? (" " + portField.value)  : "");
                } else {
                    configuration.value = "";
                }
    ...
    FileDialog {
        id: fileDialog
        title: i18nc("@title:dialog", "Open Proxy Auto-Configuration File")
